...
# ==============================================================================
# PUBLIC DEMO ENDPOINTS FOR FIRS REVIEWER (NO AUTHENTICATION REQUIRED)
# ==============================================================================

@router.post("/demo/transmission/submit")
async def demo_submit_invoice_transmission(request: InvoiceTransmissionRequest):
    """
    DEMO: Submit invoice for transmission to FIRS (No Authentication Required).
    
    This is a public demo endpoint for FIRS reviewer testing.
    """
    try:
        logger.info(f"DEMO: Starting invoice transmission for IRN: {request.irn}")
        ...
        logger.error(f"DEMO: Invoice transmission failed: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Transmission failed: {str(e)}"
        )
...
@router.get("/demo/transmission/status/{irn}")
async def demo_get_transmission_status(irn: str):
    """
    DEMO: Get transmission status for a specific invoice (No Authentication Required).
    """
    try:
        logger.info(f"DEMO: Checking transmission status for IRN: {irn}")
        ...
        logger.error(f"DEMO: Transmission status check failed: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Status check failed: {str(e)}"
        )
...
@router.post("/demo/reporting/generate")
async def demo_generate_invoice_report(request: ReportingRequest):
    """
    DEMO: Generate invoice reports for FIRS compliance (No Authentication Required).
    """
    try:
        logger.info(f"DEMO: Generating {request.report_type} report")
...
@router.get("/demo/reporting/dashboard")
async def demo_get_reporting_dashboard():
    """
    DEMO: Get reporting dashboard with key metrics (No Authentication Required).
    """
    try:
        from datetime import datetime, timedelta
        ...
        logger.error(f"DEMO: Dashboard generation failed: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Dashboard generation failed: {str(e)}"
        )
...
@router.put("/demo/update/invoice")
async def demo_update_invoice_data(request: InvoiceUpdateRequest):
    """
    DEMO: Update existing invoice data in FIRS (No Authentication Required).
    """
    try:
        logger.info(f"DEMO: Updating invoice {request.irn} - type: {request.update_type}")
        ...
        logger.error(f"DEMO: Invoice update failed: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Update failed: {str(e)}"
        )