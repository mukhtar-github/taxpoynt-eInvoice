"""
Railway-Optimized Health Check System for Blue-Green Deployments.

This module provides minimal, fast health checks specifically designed for Railway's
deployment requirements while maintaining comprehensive monitoring capabilities.
"""
...
# Import only essential modules to avoid startup failures
import logging
...
async def railway_readiness() -> Dict[str, Any]:
    """
...
async def railway_startup() -> Dict[str, Any]:
    ...
    try:
        ...
        # Check 3: Critical imports (non-blocking)
        try:
            # Test critical imports without actually executing them
            critical_modules = ["fastapi", "sqlalchemy", "pydantic"]
            import_status = {}
            for module in critical_modules:
                try:
                    ...
                    # Don't fail startup for missing optional modules
            
            startup_checks["imports"] = {
                "healthy": True,  # Don't fail for import issues during startup
                "modules": import_status
            }
        except Exception as e:
            startup_checks["imports"] = {"healthy": True, "error": str(e)}
...
# Safe check functions with timeouts and error handling
